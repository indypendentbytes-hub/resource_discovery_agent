...
async function runVerificationAgent(client, context, upstream, model) {
  ...
  const result = await client.responses.create({
    ...
    instructions: agentInstructions(agent),
    tools: [WEB_TOOL],
    input: `USER QUESTION:\n${context.query}\n\nLOCAL ROUTING SUMMARY:\n${context.routingSummary || "None"}\n\nFINDINGS TO AUDIT:\n${upstreamText}\n\nRDA SKILL CONTEXT:\n${skillContext}\n\nAudit every material claim that affects eligibility, availability, deadlines, geography, requirements, or recommended action. Verify against primary sources where possible. Return corrected findings, unresolved contradictions, stale or unsupported claims, and confidence notes.`,
  });
...
async function runPathwayAgent(client, context, upstream, model) {
  ...
  const result = await client.responses.create({
    model,
    instructions: agentInstructions(agent),
    input: `USER QUESTION:\n${context.query}\n\nLOCAL ROUTING SUMMARY:\n${context.routingSummary || "None"}\n\nVERIFIED UPSTREAM FINDINGS:\n${upstreamText}\n\nRDA SKILL CONTEXT:\n${skillContext}\n\nBuild an ordered pathway using only supported findings. Explicitly identify blockers, prerequisites, parallel actions, decision points, handoffs, and the single best next action.`,
  });
